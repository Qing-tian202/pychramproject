# test_allure4.py - 带Allure装饰器
import pytest
import allure
import os
from typing import Optional
from common import build_add_data, build_multy_data


def add(x: Optional[float | int] = 0, y: Optional[float | int] = 0) -> float | int:
    """加法函数"""
    result = round(x + y, 2)
    return result


def multy(x: Optional[float | int] = 1, y: Optional[float | int] = 1) -> float | int:
    """乘法函数"""
    result = round(x * y, 2)
    return result
...
class TestAddFunc:
    ...
    def test_add(self, x, y, expected):
        ...
        allure.attach(f"输入: x={x}, y={y}", name="输入参数", attachment_type=allure.attachment_type.TEXT)
        allure.attach(f"输出: {result}", name="输出结果", attachment_type=allure.attachment_type.TEXT)
